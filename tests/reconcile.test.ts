...
describe("reconcile — parseOwnerRepoFromRemoteUrl", () => {
  it("parses https URL", () => {
...
describe("reconcile — schema back/forward compat", () => {
  ...
  it("round-trips a v2 binding with reconcile fields", () => {
    const tmp = useTmpDir();
...
describe("reconcile — scanTreeNodes", () => {
...
describe("reconcile — default run", () => {
  ...
  it("first-run: pins binding to current HEAD without proposals", async () => {
    const tmp = useTmpDir();
    makeTreeShell(tmp.path);
    const binding: Omit<TreeBindingState, "schemaVersion"> = {
      bindingMode: "standalone-source",
    ...
    const shellRun = makeOkShellRun({
      "gh api /repos/alice/source/commits/HEAD": {
        stdout: `${head}\n`,
        stderr: "",
        code: 0,
      },
    ...
    expect(after?.lastReconciledSourceCommit).toBe(head);
    expect(after?.lastReconciledAt).toBe("2026-04-10T10:00:00.000Z");
    const proposalsDir = join(tmp.path, ".first-tree", "proposals", "source-111");
    expect(existsSync(proposalsDir)).toBe(false);
  });

  it("stale + --propose: writes proposal files via deterministic fallback", async () => {
    const tmp = useTmpDir();
    makeTreeShell(tmp.path);
    ...
    const shellRun: ShellRun = async (command, args) => {
      ...
      if (command === "claude") {
        // simulate claude CLI missing / non-json — force fallback
  ...
  it("deterministic fallback when claude CLI is missing groups by dir", async () => {
    const tmp = useTmpDir();
    makeTreeShell(tmp.path);
    const fromSha = "aa".repeat(20);
    const toSha = "bb".repeat(20);
    writeTreeBinding(tmp.path, "source-333", {
      bindingMode: "standalone-source",
      entrypoint: "/repos/source",
      lastReconciledSourceCommit: fromSha,
      remoteUrl: "https://github.com/alice/source.git",
              ...
              commits: [
                {
                  sha: "1".repeat(40),
                  commit: {
                    message: "feat(pkg-a): thing",
                    author: { name: "a", date: "2026-04-01T00:00:00Z" },
                  },
                  files: [{ filename: "pkg-a/src/x.ts" }],
                },
                {
                  sha: "2".repeat(40),
                  commit: {
                    message: "feat(pkg-b): thing",
                    author: { name: "a", date: "2026-04-02T00:00:00Z" },
                  },
                  files: [{ filename: "pkg-b/src/y.ts" }],
                },
              ],
            }),
            stderr: "",
    ...
    expect(code).toBe(0);
    const dir = join(tmp.path, ".first-tree", "proposals", "source-333");
...
describe("reconcile CLI", () => {
  it("prints USAGE on --help", async () => {
    ...
    const code = await runReconcileCli(["--help"]);
    expect(code).toBe(0);
    expect(logSpy).toHaveBeenCalledWith(RECONCILE_USAGE);
    logSpy.mockRestore();
  });
  ...
  it("exits 1 when --tree-path target is not a tree repo", async () => {
    ...
    const code = await runReconcileCli(["--tree-path", tmp.path], { shellRun });
    expect(code).toBe(1);
    errSpy.mockRestore();
  });
});
