...
import { createBus, type BusEvent } from "../../src/daemon/bus.js";
import {
  Dispatcher,
...
import { tryClaim } from "../../src/daemon/claim.js";
...
describe("Dispatcher execution", () => {
  ...
  it("aborts the spawner's signal when the task timeout fires", async () => {
    let observedSignal: AbortSignal | undefined;
    const spawner: AgentSpawner = ({ signal }) =>
      new Promise((_resolve, reject) => {
        observedSignal = signal;
        signal?.addEventListener(
          "abort",
          () => reject(new Error("aborted by spawner")),
          { once: true },
        );
      });
    const env = setupDispatcher({ spawner, taskTimeoutMs: 40 });
    env.dispatcher.submit(fakeCandidate());
    await waitForCompletions(env, 1, 1_000);
    expect(env.completions[0].phase).toBe("timed_out");
