...
describe("startHttpServer — route contracts", () => {
  ...
  it("GET / returns the dashboard HTML with the contract headers", async () => {
    ...
    expect(res.body.includes("<title>auto</title>")).toBe(true);
...
describe("startHttpServer — graceful shutdown via AbortSignal", () => {
  ...
  it("server.stop() closes the listener without requiring the shared signal to abort", async () => {
    writeFileSync(ctx.inbox, '{"last_poll":"x","notifications":[]}', "utf-8");
    const manual = makeManualBus();
    const controller = new AbortController();
    const server = await startHttpServer({
    ...
    // Caller invokes stop() directly; the shared signal is still live.
    // stop() must close the listener and resolve done() without hanging.
    await server.stop();
