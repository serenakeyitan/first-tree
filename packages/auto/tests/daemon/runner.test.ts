...
import * as identityModule from "../../src/daemon/identity.js";
import * as httpModule from "../../src/daemon/http.js";
import * as pollerModule from "../../src/daemon/poller.js";
import {
  parseDaemonArgs,
  runDaemon,
} from "../../src/daemon/runner-skeleton.js";
import { extractBackendFlag } from "../../src/cli.js";

const tempRoots: string[] = [];
const ORIGINAL_AUTO_DIR = process.env.AUTO_DIR;
const ORIGINAL_AUTO_HOME = process.env.AUTO_HOME;
...
afterEach(() => {
  ...
  if (ORIGINAL_AUTO_DIR === undefined) delete process.env.AUTO_DIR;
  else process.env.AUTO_DIR = ORIGINAL_AUTO_DIR;
  if (ORIGINAL_AUTO_HOME === undefined) delete process.env.AUTO_HOME;
  else process.env.AUTO_HOME = ORIGINAL_AUTO_HOME;
...
function makeTempRoot(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `auto-daemon-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("runDaemon end-to-end skeleton", () => {
  ...
  it("refreshes the service lock heartbeat and writes runtime status while running", async () => {
    const autoDir = makeTempRoot("status");
    process.env.AUTO_DIR = autoDir;
    delete process.env.AUTO_HOME;
    process.env.PATH = "";
    ...
    const runnerHome = join(autoDir, "runner");
    const lockPath = join(
      runnerHome,
      "locks",
...
describe("cli dispatcher routes run / run-once / daemon to the TS runner", () => {
  ...
  it("routes `daemon` to runDaemon with once=false", async () => {
    ...
    const { runAuto } = await import("../../src/cli.js");
    await runAuto(["daemon", "--poll-interval-secs", "30"], () => {});
  ...
  it("routes `run` to runDaemon with once=false and strips any stray --backend flag", async () => {
    ...
    const { runAuto } = await import("../../src/cli.js");
    await runAuto(["run", "--backend=rust", "--verbose"], () => {});
