/**
 * Exhaustive coverage for `classifyAutoStatus`, mirroring the state
...
describe("classifier — precedence rules (spec §2)", () => {
  it("rule 1: breeze:done wins over everything", () => {
    ...
    expect(
      classifyAutoStatus({ labels: ["breeze:done"], ghState: "MERGED" }),
    ).toBe("done");
  });

  it("rule 2: MERGED/CLOSED derives done absent breeze:done", () => {
    expect(
      classifyAutoStatus({ labels: [], ghState: "MERGED" }),
    ).toBe("done");
    expect(
      classifyAutoStatus({ labels: [], ghState: "CLOSED" }),
    ).toBe("done");
  });

  it("rule 2: MERGED/CLOSED wins over breeze:human and breeze:wip", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:human"], ghState: "MERGED" }),
    ).toBe("done");
    expect(
      classifyAutoStatus({ labels: ["breeze:wip"], ghState: "CLOSED" }),
    ).toBe("done");
  });

  it("rule 3: breeze:human wins on OPEN", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:human"], ghState: "OPEN" }),
    ).toBe("human");
  });
  ...
  it("rule 4: breeze:wip on OPEN derives wip", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:wip"], ghState: "OPEN" }),
    ).toBe("wip");
  });

  it("rule 5: no breeze:* labels on OPEN → new", () => {
    expect(classifyAutoStatus({ labels: [], ghState: "OPEN" })).toBe("new");
  });
  ...
  it("rule 5: breeze:new label alone does NOT override (§2 subtleties)", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:new"], ghState: "OPEN" }),
    ).toBe("new");
  });
});
...
describe("classifier — null / undefined ghState (Discussion et al.)", () => {
  it("null ghState + no breeze labels → new", () => {
    expect(classifyAutoStatus({ labels: [], ghState: null })).toBe("new");
    expect(classifyAutoStatus({ labels: [], ghState: undefined })).toBe("new");
  });
  it("null ghState + breeze:wip → wip (labels still drive derivation)", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:wip"], ghState: null }),
    ).toBe("wip");
  });
  it("null ghState + breeze:human → human", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:human"], ghState: null }),
    ).toBe("human");
  });
  it("null ghState + breeze:done → done", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:done"], ghState: null }),
    ).toBe("done");
  });
});
...
describe("classifier — observable state-machine transitions (spec §1)", () => {
  ...
  it("new → done (via gh_state): state flips to MERGED/CLOSED", () => {
    expect(classifyAutoStatus({ labels: [], ghState: "MERGED" })).toBe("done");
    expect(classifyAutoStatus({ labels: [], ghState: "CLOSED" })).toBe("done");
  });
  ...
  it("wip → done (via gh_state MERGED/CLOSED)", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:wip"], ghState: "MERGED" }),
    ).toBe("done");
  });
  ...
  it("human → done (gh_state MERGED/CLOSED)", () => {
    expect(
      classifyAutoStatus({ labels: ["breeze:human"], ghState: "CLOSED" }),
    ).toBe("done");
  });
