 ...
 * commands. We run them against a temporary `$AUTO_HOME` and assert
...
function makeHome(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `auto-cmd-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("runDoctor", () => {
  it("prints the one-screen diagnostic regardless of identity", async () => {
    ...
    expect(out).toContain("auto-runner doctor");
...
describe("runStatus", () => {
  it("reports runtime/status.env keys when present", async () => {
    ...
    expect(code).toBe(0);
    const out = lines.join("\n");
    expect(out).toContain("auto-runner status");
...
describe("runInstall", () => {
  it("refuses to install-start without an explicit repo scope", () => {
    ...
    const code = runInstall([], {
      autoDir: makeHome("install-missing-scope"),
      write: (line) => lines.push(line),
      checkCommand: () => true,
      checkGhAuth: () => true,
      spawn,
  ...
  it("re-invokes the current CLI for `auto start` instead of shelling to PATH", () => {
    const lines: string[] = [];
    const spawn = vi.fn(() => ({ status: 0 })) as unknown as typeof import("node:child_process").spawnSync;
    const code = runInstall(["--allow-repo", "owner/repo"], {
      autoDir: makeHome("install"),
      write: (line) => lines.push(line),
      checkCommand: () => true,
      checkGhAuth: () => true,
      spawn,
      startCommand: {
        ...
        args: ["/tmp/first-tree/dist/cli.js", "auto", "start"],
      },
    });
    expect(code).toBe(0);
...
describe("cli routing", () => {
  it("maps doctor/status/cleanup/start/stop/poll-inbox to TS specifiers", async () => {
    ...
    expect(cli.AUTO_USAGE).toContain("doctor");
    expect(cli.AUTO_USAGE).toContain("poll-inbox");
