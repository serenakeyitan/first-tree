...
import { runStatusManager } from "../../src/commands/status-manager.js";
...
function mkAutoDir(): { dir: string; inbox: string; activity: string; claims: string } {
  const dir = mkdtempSync(join(tmpdir(), "auto-sm-"));
...
describe("status-manager: help", () => {
  it("prints usage and exits 0", async () => {
    ...
    expect(code).toBe(0);
    expect(stdout[0]).toMatch(/Usage: auto-status-manager/u);
  });
});
