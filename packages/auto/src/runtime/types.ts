/**
 * Runtime-validated types for the shared auto store.
 *
 ...
 * existing `~/.first-tree/auto/inbox.json` and `~/.first-tree/auto/activity.log` files
...
export const AutoStatusSchema = z.enum(["new", "wip", "human", "done"]);
...
export const InboxEntrySchema = z.object({
  ...
  breeze_status: AutoStatusSchema,
});
...
export const ALL_AUTO_LABELS = [
  "breeze:new",
  "breeze:wip",
  "breeze:human",
  "breeze:done",
] as const;
export type AutoLabel = (typeof ALL_AUTO_LABELS)[number];
