 ...
 *
 * This file is its own dist bundle (`dist/auto-statusline.js`). It is
 ...
 *
 * Behaviour is a line-for-line port of the historical `breeze-status` /
 * `breeze-statusline-wrapper` bash scripts. Stdin is drained silently
 * per the Claude Code statusline contract.
 *
 * Input: swallow stdin (Claude Code statusline hooks receive JSON).
 * Output: a single line to stdout, e.g.
 *   `/auto: ⚠ 2 need-you · 52 PRs · 3 issues (+1 new)`
...
/** 10 minutes; matches the legacy `bin/breeze-status:25` threshold. */
const STALE_THRESHOLD_SECS = 600;

function resolveAutoDir(): string {
  const override = process.env.AUTO_DIR;
  if (override && override.length > 0) return override;
  return join(homedir(), ".first-tree", "auto");
}
...
export function renderStatusline(
  ...
  return { line: `/auto: ${humanPart}${newSummary}${suffix}`, ring };
}
...
export function main(
  ...
  const autoDir = resolveAutoDir();
  const inboxPath = join(autoDir, "inbox.json");
  const bellStatePath = join(autoDir, ".bell_state");
  ...
  try {
    ...
    if (nowSecs - mtimeSecs > STALE_THRESHOLD_SECS) {
      process.stdout.write("/auto: stale (poller not running?)\n");
      return 0;
    }
  } catch {
...
// Run only when this file is executed directly (i.e. `node dist/auto-statusline.js`),
// not when imported.
const isDirectRun =
  typeof process !== "undefined" &&
  Array.isArray(process.argv) &&
  process.argv[1] !== undefined &&
  /auto-statusline(\.[cm]?js)?$/u.test(process.argv[1]);
if (isDirectRun) {
  process.exit(main());
}
